...
if (isset($_POST['ajouter']))        //ajout des visuel
{
    ...
    $stmt->bind_param("isssssi", $id, $vis_desc, $nom_fichier, $thedate, $visi, $pseudo, $cat);
...
if (isset($_POST['modifier'])) {
    if ($_POST['modif_vis'] == "non") {
        $stmt = $mysqli->prepare("  UPDATE t_visuel_vis 
                                    SET vis_descriptif=? , vis_visibilite=?, vis_date_ajout = ?, cat_id= ?
                                    where vis_id=?;");
        $stmt->bind_param("sssii", $desc, $visi, $thedate, $cat, $id);
        $desc = $_POST['desc'];
        $visi = $_POST['visi'];
        $thedate = $_POST['thedate'];
        $cat = $_POST['cat'];
        ...
        //MAJ des donnes
        $stmt = $mysqli->prepare("UPDATE t_visuel_vis 
        SET vis_descriptif=? , vis_nom_fichier=? , vis_visibilite=?, vis_date_ajout = ?, cat_id= ?
        where vis_id=?;");
        $stmt->bind_param("ssssii", $desc, $new_file_name, $visi, $thedate, $cat, $id);
        $desc = $_POST['desc'];
